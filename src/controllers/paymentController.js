// ✅ Updated PaymentController.js with QR Image Fix in manual payment render
...
const initiateManualUPIPayment = async (req, res) => {
  ...
  const momo = {
    ...
    qr_image: bank_recharge_momo_data?.qr_code_image || "",
  };

  return res.render("wallet/manual_payment.ejs", {
    Amount: query?.am,
    UpiId: momo.upi_id,
    QrImage: momo.qr_image,
  });
};
...
const initiateManualUSDTPayment = async (req, res) => {
  try {
    const query = req.query;
    const [bank_recharge_usdt] = await connection.query(
      "SELECT * FROM bank_recharge WHERE type = 'usdt' LIMIT 1"
    );

    if (!bank_recharge_usdt.length) {
      return res.send("USDT payment config not found.");
    }

    const usdt = {
      usdt_wallet_address: bank_recharge_usdt[0].stk || "",
      qr_image: bank_recharge_usdt[0].qr_code_image || "",
    };

    return res.render("wallet/usdt_manual_payment.ejs", {
      Amount: query?.am,
      WalletAddress: usdt.usdt_wallet_address,
      WalletQR: usdt.qr_image,
    });
  } catch (err) {
    console.error("Error in initiateManualUSDTPayment:", err);
...
const addManualUSDTPaymentRequest = async (req, res) => {
  try {
    ...
    let utr = parseInt(data.utr);
    const minimumMoneyAllowedu = parseInt(process.env.MINIMUM_USDT || 460);

    if (!money || !(money >= minimumMoneyAllowedu)) {
      return res.status(400).json({
        message: `Money is Required and it should be ₹${minimumMoneyAllowedu} or ${(
          minimumMoneyAllowedu / 92
        ).toFixed(2)} or above!`,
        status: false,
        timeStamp: timeNow,
      });
